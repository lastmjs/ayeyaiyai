...
impl<'a> FunctionCompiler<'a> {
            ...
            Expression::Call { callee, arguments } => {
                ...
                self.resolve_static_primitive_expression_with_context(
                    ...
                    callee_function_name
                        .as_deref()
                        .or(self.current_user_function_name.as_deref()),
                )
    ...
    fn resolve_static_object_identity_expression(
        &self,
        expression: &Expression,
    ) -> Option<Expression> {
        if self
        ...
        match materialized {
            ...
            Expression::Binary { op, left, right } => match op {
                BinaryOp::Equal | BinaryOp::LooseEqual => {
                    ...
                    {
                        return Some(left_identity == right_identity);
                    }
    ...
    pub(in crate::backend::direct_wasm) fn resolve_static_array_is_array_call_result(
        ...
        Some(
            !matches!(argument, Expression::Identifier(name) if self.local_typed_array_view_bindings.contains_key(name))
                && self
    ...
    pub(in crate::backend::direct_wasm) fn infer_typeof_operand_kind(
        ...
        match expression {
            Expression::This => self.top_level_function.then_some(StaticValueKind::Object),
            Expression::Identifier(name)
                if name == "NaN" && self.is_unshadowed_builtin_identifier(name) =>
            {
    ...
    pub(in crate::backend::direct_wasm) fn lookup_identifier_kind(
        ...
        if self.deleted_builtin_identifiers.contains(name) {
            return None;
        }
    ...
    pub(in crate::backend::direct_wasm) fn infer_value_kind(
        ...
        match expression {
            ...
            Expression::Member { object, property } => {
                ...
                if let Expression::Identifier(object_name) =

...
pub use direct_wasm_backend::emit_wasm;
pub use direct_wasm_backend::emit_wasm_with_reason;

pub fn compile_file(path: &Path, options: &CompileOptions) -> Result<()> {
    let program = frontend::bundle_script_entry(path)?;
    aot::validate_refined_aot(&program)?;
    let program = aot::lower_static_function_constructors(program)?;
    if direct_wasm_backend::compile_if_supported(&program, options)? {
        return Ok(());
    }
...
pub fn compile_source_with_reason(source: &str) -> std::result::Result<(), String> {
    ...
    aot::validate_refined_aot(&program).map_err(|_| "aot validation failed".to_string())?;
    let program = aot::lower_static_function_constructors(program)
        .map_err(|_| "aot lowering failed".to_string())?;
    match direct_wasm_backend::emit_wasm_with_reason(&program) {
        Ok(_) => Ok(()),
...
mod tests {
    ...
    use crate::{
        ...
        hir::{Expression, Statement, UpdateOp},
    };

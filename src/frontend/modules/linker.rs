...
impl ModuleLinker {
    pub(super) fn bundle_entry(&mut self, path: &Path) -> Result<Program> {
    ...
    pub(super) fn bundle_script_entry(&mut self, path: &Path) -> Result<Program> {
    ...
    pub(super) fn ensure_module_slot(&mut self, path: &Path) -> Result<usize> {
    ...
    pub(super) fn load_module(&mut self, path: &Path) -> Result<usize> {
    ...
    pub(super) fn compute_static_load_order(&self, entry_index: usize) -> Vec<usize> {
        fn visit(
            linker: &ModuleLinker,
            module_index: usize,
